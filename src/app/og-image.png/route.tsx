...
export async function GET() {
  try {
    ...
    const description =
      "Vývoj moderních webových aplikací a webů s důrazem na výkon, design a uživatelskou přívětivost.";
    ...
    const fontBoldUrl = spaceGroteskFont
      .match(/src: url\(([^)]+)\)/g)?.[1]
      ?.match(/src: url\(([^)]+)\)/)?.[1];

    // Fetch the actual font files
    const fontRegular = fontRegularUrl
      ? await fetch(new URL(fontRegularUrl, import.meta.url)).then((res) =>
          res.arrayBuffer()
        )
      : null;
    const fontBold = fontBoldUrl
      ? await fetch(new URL(fontBoldUrl, import.meta.url)).then((res) =>
          res.arrayBuffer()
        )
      : null;
